...
Deno.test("should handle function handovers", async () => {
    ...
    const bloomFilter = new BloomFilter(10, 1)

    const exampleArray = [6, 9, 10]

    for (const entry of exampleArray) {
        bloomFilter.add(entry, (x: number) => x * 2 % 10, (x: number) => x * 3 % 10, (x: number) => x * 4 % 10)
    }

    assertEquals(bloomFilter.test(11, (x: number) => x * 2 % 10, (x: number) => x * 3 % 10, (x: number) => x * 4 % 10), EBloomBool.NO)
    assertEquals(bloomFilter.test(6, (x: number) => x * 2 % 10, (x: number) => x * 3 % 10, (x: number) => x * 4 % 10), EBloomBool.PERHAPS)
    assertEquals(bloomFilter.test(3, (x: number) => x * 2 % 10, (x: number) => x * 3 % 10, (x: number) => x * 4 % 10), EBloomBool.NO)
})

Deno.test("should return EBloomBool.NO if entry is not yet added", async () => {

    const bloomFilter = new BloomFilter(128, 1)
